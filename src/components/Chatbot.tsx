...
//   2) "Don't know what to order, we got you!" → asks budget → recommends.
// All AI calls go through the recommendOrder server function (LOVABLE_API_KEY stays on server).
...
export function Chatbot() {
  const { food, beverages } = useCart();
  const callAi = useServerFn(recommendOrder);

  const [open, setOpen] = useState(false);
  // Mode "budget" tells the next user message to be parsed as a budget number.
  ...
  // Send a turn to the model and append the reply.
  const ask = async (newMessages: Msg[], budget?: number) => {
    setBusy(true);
    try {
      const { reply } = await callAi({ data: { messages: newMessages, menu, budget } });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // Budget mode → parse a number from the user's reply.
    if (mode === "budget") {
      ...
      setMode("chat");
      await ask(
        [...next, { role: "user", content: `Pick the best combo for ₹${budget}.` }],
        budget,
      );
      return;
    }
  ...
  // Quick action: ask for today's pick without typing.
  ...
  // Quick action: start the budget flow.
  const helpMeChoose = () => {
    if (busy) return;
    setMode("budget");
    setMessages((p) => [
        ...
        <div className="fixed inset-x-2 bottom-2 sm:inset-auto sm:bottom-6 sm:right-6 sm:w-[380px] z-40 rounded-2xl border border-border bg-card/95 backdrop-blur-xl shadow-glow flex flex-col max-h-[80vh] animate-fade-up">
          ...
          <header className="flex items-center gap-2 p-3 border-b border-border">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[11px] text-muted-foreground">Your canteen pal</div>
            </div>
            <button
